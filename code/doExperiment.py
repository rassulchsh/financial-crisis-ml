...
from data_pipeline.utils import CustomGroupKFold, CustomTimeSeriesSplit, InSampleSplit, forecastExp
...
class Experiment:
    ...
    def __init__(self, data, models, expType, rs=1):
        ...
        validModels = ["Random Assignment", "Logit", "RandomForest",
                       "ExtraTrees", "SVM", "NeuralNet", "KNeighbors"]
        validExpTypes = ["InSample", "CrossVal", "Forecast", 0, 1, 2]
        unvalidModels = [
            model for model in self.models if model not in validModels]
    ...
    def run(self, n=1, disableTqdm=False):
        ...
        self.searchRes = []
        if self.expType == "InSample" or self.expType == "Forecast":
            n = 1
        ...
        for model in self.models:
            ...
            for i in tqdm(range(1, n+1), desc=self.data.name + ": " + model, disable=disableTqdm):
                ...
                for ixTrain, ixTest in self.iterator.split(self.data.df):
                    ...
                    self.searchRes.append(
                        (str(model), pipe.named_steps["BestEstimator"].best_params_))
    ...
    def rocGraph(self, save=False):
        ...
        plot.set(xlabel="False Positive Rate (FPR)",
                 ylabel="True Positive Rate Rate (TPR)")
        labels = [
            f"{mo}: (AUC = {round(self.auc[self.auc.Model == mo].AUC.iloc[0], 3)})" for mo in self.models]
    ...
    def ALE(self, modelTypes, var):
        ...
        for modelType in modelTypes:
            ...
            def predict_crisis(x): return model.predict_proba(x)[:, 1]
        ...
        for model in exps.keys():
            plots[model] = plot_ale(
                exps[model], n_cols=4, ax=ax, line_kw={"label": model})
        fig.delaxes(ax[4, 2])
