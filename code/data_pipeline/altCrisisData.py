...
def getLaevenValencia(folder=None, vers=2020):
    ...
    if folder == None:
        ...
        df_alt = pd.read_excel(folder + r"\LaevenValencia" + str(vers) +
                               "_crises.xlsx", sheet_name="Crisis Years").drop(0)

    df_alt = df_alt.set_axis(["Country", "crisis_banking", "crisis_currency",
                             "crisis_debt", "restructuring"], axis="columns")
    ...
    for index, row in df_alt.iterrows():
        if row["crisis_banking"] is not np.NaN:
            new.loc[(new.iso == row["iso3"]) & (new.year.astype(str).isin(
                str(row["crisis_banking"]).split(sep=", "))), "crisis_banking"] = 1
...
def getESRB(folder=None, vers=2021):
    ...
    if folder == None:
        ...
        df_alt = pd.read_excel(link, sheet_name="Systemic crises").drop(
            [0, 51, 52, 53, 54])
    else:
        df_alt = pd.read_excel(folder + r"\esrb" + str(vers) + ".xlsx",
                               sheet_name="Systemic crises").drop([0, 51, 52, 53, 54])

    df_alt = df_alt[["Country", "Start date"]].dropna(
        subset=["Start date"]).copy()
    ...
    new = pd.concat((df_alt, new)).sort_values(
        ["year", "iso"]).reset_index(drop=True)
